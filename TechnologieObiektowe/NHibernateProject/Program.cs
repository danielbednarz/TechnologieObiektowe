...
//    })
//    .Build();
using (var session = NHibernateExtensions.OpenStatelessSession("Server=OMEN-15\\SQLINSTANCE;Database=NHDatabase;Trusted_Connection=True;Encrypt=False;"))
{
    ITransaction transaction = session.BeginTransaction();
    try
    {
        var seedElapsedTime = StopwatchHelper.MeasureExecutionTime(() => DbInitializer.Seed(session, transaction));
